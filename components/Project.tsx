...
const Project: FunctionComponent<PropTypes> = ({ project }) => {
  ...
  return (
    <div className='bg-white shadow-lg rounded-lg flex flex-col px-8 py-8 sm:px-16 sm:py-12 gap-4 max-w-xl relative'>
      <span className='hidden sm:absolute sm:flex top-4 right-4 text-black-90 hover:text-secondary transition-all duration-200 hover:cursor-pointer text-h5'><a href={link}><MdOpenInNew /></a></span>
      <h3 className='md:text-h3 text-h3-mobile text-black-90'>{title}</h3>
      <div className='flex flex-row w-full h-full justify-center'>
        <img src={collage.default.src} className='object-scale-down' alt="Project Image 1"/>
      </div>
